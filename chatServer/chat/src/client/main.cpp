...
// 发送带4字节长度头的JSON消息
bool sendJsonWithLength(int clientfd, const json &js)
{
    string body = js.dump();
    ...
    ::memcpy(&buf[sizeof(uint32_t)], body.data(), bodyLen);

    size_t total = 0;
    while (total < buf.size())
    {
        ssize_t n = send(clientfd, buf.data() + total, buf.size() - total, 0);
        if (n <= 0)
        {
            return false;
        }
...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        cerr << "command invalid! example: ./ChatClient 127.0.0.1 8000\nip + port is your nginx server's ip and port" << endl;
        exit(-1);
    }
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            if (!sendJsonWithLength(clientfd, js))
            {
...
void readTaskHandler(int clientfd)
{
    ...
    {
        char buffer[1024] = {0};
        int len = recv(clientfd, buffer, sizeof(buffer), 0); // 阻塞读
        if (len <= 0)
        {
            close(clientfd);
            exit(-1);
        ...
        {
            uint32_t netlen = 0;
            ::memcpy(&netlen, recvBuffer.data(), sizeof(netlen));
            ...
            if (recvBuffer.size() < sizeof(uint32_t) + bodyLen)
            {
                // 数据还不够一条完整消息
                break;
            }

            std::string jsonStr = recvBuffer.substr(sizeof(uint32_t), bodyLen);
            recvBuffer.erase(0, sizeof(uint32_t) + bodyLen);
